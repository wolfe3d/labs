...
function (sym, modelIndex, sgName, symOp, pt1, pt2, drawID, scaleFactor, nth, isFull, isForModel, options) {
var info = null;
var cellInfo = null;
...
return info;
}, "JS.Symmetry,~N,~S,~N,JU.P3,JU.P3,~S,~N,~N,~B,~B,~N");
...
if (type == 1275068418 && nth < 0 && op <= 0) {
var lst =  new JU.Lst ();
for (var i = 0; i < info.length; i++) lst.add (JS.SymmetryDesc.getInfo (info[i], 1275068418));
...
var sginfo = this.getSpaceGroupInfo (sym, modelIndex, null, symOp, pt1, pt2, drawID, scaleFactor, nth, false, true, options);
